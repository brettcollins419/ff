...
session.auth = ('horribleoracle', 'MizzouOracle')

address = 'https://premium.pff.com/api/v1/facet/rushing/summary?league=nfl&season=2019&week=2&export=true'
address2 = 'https://horribleoracle:MizzouOracle@premium.pff.com/api/v1/facet/rushing/summary?league=nfl&season=2019&week=1&export=true'

csv = requests.get(address)

csv = requests.get(address, auth=HTTPBasicAuth('horribleoracle', 'MizzouOracle'), verify = False)
data = pd.read_csv(StringIO(session.get(address).text))

x = pd.read_csv(address2)
